...
class K3H3584MoeFinalizeAllReduceRMSNormHTDeviceKernel:
    """FlashInfer HT device kernel with tail-predicated reduction shards.

    This is the FlashInfer 0.6.18 HT kernel with one shape-generalization:
    reduction shards may end in a partial warp. Every other producer,
    consumer, publisher, RMSNorm, and synchronization path is kept identical
    ...
    def __init__(
        ...
        *,
        ...
        if self.rms_threads_per_token % WARP_SIZE:
            raise ValueError("RMS threads per token must be a whole number of warps")
